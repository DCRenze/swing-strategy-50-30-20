"""Phase 5: can Sleeve A buy a higher win rate by avoiding jumpy stocks?

Pre-registered in results/VOLATILITY_HYPOTHESIS.md - read that first; the pass
and kill criteria were fixed before this ran, and this phase deliberately
reports TWO verdicts (does it raise the win rate / should we deploy it).

H2: restricting Sleeve A entries to names whose ATR(10) is at or below a fixed
fraction of price raises the sleeve's win rate. Whether it raises the sleeve's
or the book's RETURN is the open question - calm names win more often but pay
less per win (avg return +0.24% at <2% ATR vs +0.40% at >4%), so this is a
trade-off to price, not a free lunch.

This is entry selection only. Stretch, trend SMA, the limit offset, the exit
rule, the 15-day time stop and the slot count all stay at their validated
values; max_atr_pct=None reproduces the deployed config exactly.
...
A threshold has to clear two bars, not one:
  - the A sleeve itself must improve, and
...
Run:  python -m backtest.phase5_volatility --stage is
      python -m backtest.phase5_volatility --stage oos --confirm maxatr2.5
"""
...
# label -> params layered on the validated A baseline.
#
# Grid set from the SIGNAL distribution before any performance result: Sleeve A
# signals run a median ATR of 2.24% of price (p25 1.74%, p75 2.98%, p90 3.99%).
# The caps therefore span roughly p40 to p95, so a plateau has room to show both
# edges. Each cap is a real treatment - 2.0% keeps 38% of signals, 2.5% keeps
# 60%, 3.0% keeps 75%, 4.0% keeps 90%.
VARIANTS: list[tuple[str, dict]] = [
    ("baseline", {}),
    ("maxatr2.0", {"max_atr_pct": 0.020}),
    ("maxatr2.5", {"max_atr_pct": 0.025}),
    ("maxatr3.0", {"max_atr_pct": 0.030}),
    ("maxatr3.5", {"max_atr_pct": 0.035}),
    ("maxatr4.0", {"max_atr_pct": 0.040}),
    ("maxatr5.0", {"max_atr_pct": 0.050}),
]
...
def run(stage: str, confirm: list[str]) -> dict:
    ...
    print(f"Phase 5 volatility sweep - {tag} window, {len(labels)} config(s)", flush=True)
...
def write_report(out: dict) -> None:
    ...
    w = out["window"]
    lines = [
        f"# Phase 5 volatility sweep - {'in-sample' if stage == 'is' else 'out-of-sample'} results",
        "",
    ...
    path = RESULTS_DIR / f"PHASE5_VOLATILITY_{tag}.md"
    path.write_text("\n".join(lines) + "\n")
    print(f"wrote {path}")
...
def main() -> None:
    ...
    (RESULTS_DIR / f"phase5_volatility_{tag}.json").write_text(json.dumps(out, indent=2))
    write_report(out)
